...
async function extractAndRun(
  ...
  try {
    ...
    if (process.env.NEW_VIBE_KANBAN_DEBUG || process.env.VIBE_KANBAN_DEBUG) {
    ...
    .then((latest) => {
      if (latest && latest !== CLI_VERSION) {
        setTimeout(() => {
          ...
          console.log(`Run: npx new-vibe-kanban@latest`);
        }, 2000);
      }
    })
    .catch(() => {});
...
async function runMain(desktopMode: boolean): Promise<void> {
  ...
  if (desktopMode && tauriPlatform) {
    try {
      console.log(
        `Starting new-vibe-kanban desktop v${CLI_VERSION}${modeLabel}...`,
      );
  ...
  console.log(`Starting new-vibe-kanban v${CLI_VERSION}${modeLabel}...`);
...
function runOrExit(task: Promise<void>): void {
  void task.catch((err: unknown) => {
    ...
    if ((process.env.NEW_VIBE_KANBAN_DEBUG || process.env.VIBE_KANBAN_DEBUG) && err instanceof Error) {
...
async function main(): Promise<void> {
  ...
  const cli = cac("new-vibe-kanban");

  cli
    .command("[...args]", "Launch the local new-vibe-kanban app")
